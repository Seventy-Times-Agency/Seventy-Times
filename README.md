...
Секция `Testimonials` — горизонтальный карусель-слайдер с картинками
отзывов. Рядом с заголовком кнопка `Оставить отзыв` открывает модалку
(`href="#review"`), в которой клиент должен ввести персональный код.

Коды перечислены через запятую в env var `CLIENT_CODES`. Выдавай код
каждому реальному клиенту после завершения проекта — например
`IAA-XYZ-42`. Сравнение без учёта регистра, пробелов, дефисов и
подчёркиваний.

Обработчик — `src/app/api/review/route.ts`:

- Проверяет код клиента против `CLIENT_CODES`. Неверный код → 401 с
  подсказкой связаться в Telegram для получения кода.
- Валидный отзыв уходит в логи сервера и (если задан Telegram) — в
  тот же чат модератора.
- После подтверждения админ вручную добавляет отзыв в
  `src/data/testimonials.ts`.

Такой подход требует чуть больше ручного труда, но даёт полную
контролируемость содержимого и защищает от спама и фейковых отзывов.
